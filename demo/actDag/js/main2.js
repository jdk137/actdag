...
d3.json("exampleStateMachine.json", function(fData) {
  ...
  //添加单个节点
  $('#addNode2').click(function () {
    var random = Math.floor(Math.random() * 100000);
    dag2.addNode({id: 'n-' + random, name: 'n-' + random});
  });
  //添加已存在节点
  $('#addExistedNode2').click(function () {
    var random = Math.floor(Math.random() * 100000);
    //dag.addNode({id: 'n-' + random, name: 'n-' + random});
    dag2.addNode({id: 'phone1', name: 'phone1'});
  });
  //获取存储数据
  $('#save2').click(function () {
    var data = dag2.getSaveData();
    if (data.message === 'isolated') {
      alert('存在孤立节点或节点群，不能保存');
    } else if (data.message === 'looped') {
      alert('存在循环，不能保存');
